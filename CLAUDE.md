...
1. **Days** = `filing − initial` (UTC midnight diff, in `daysBetween`). Per spec the count *includes* the initial date and *excludes* the filing date, which is exactly the plain difference — no `+1`/`−1` adjustment. Dates are entered as `DD/MM/YYYY` via a masked text input; `parseDmyToIso` converts to `YYYY-MM-DD` and validates real-calendar dates (rejects e.g. `31/02/2024`). A calendar-icon button next to each date input triggers the native browser picker via `showPicker()`.
